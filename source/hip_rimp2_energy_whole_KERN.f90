...
#if defined(HIPBLAS)
        integer(c_int)   :: hipblas_return
        type(c_ptr)      :: hipblas_handle
!#if defined(HIPBLASXT)
!          integer(c_int),dimension(1) :: hipblasXt_deviceId
#endif
!#endif
...
#elif defined(HIPBLASXT)
        write(*,*) 'You are running the code with hipblasxt on GPU'
#else
...
#if defined(HIPBLAS)
        hipblas_return = hipblascreate_v2(hipblas_handle)
#elif defined(HIPBLASXT)
        hipblas_return = hipblasXtcreate(hipblas_handle)
        hipblasXt_deviceId(1) = 0      ! only for one GPU
        hipblas_return = hipblasXtDeviceSelect(hipblas_handle, 1, hipblasXt_deviceId)  ! only for one GPU
        hipblas_return = hipblasXtSetBlockDim(hipblas_handle, 2048)
#endif
...
#if defined(HIPBLAS)
        hipblas_return =hipblasdestroy_v2(hipblas_handle)
#elif defined(HIPBLASXT)
        hipblas_return = hipblasXtdestroy(hipblas_handle)
#endif
...
#if defined(INTEL_OFFLOAD) || defined(NVBLAS) || defined(HIPBLAS) || defined(HIPBLASXT)
...
#if defined(NVBLAS) || defined(HIPBLAS) || defined(HIPBLASXT)
        !$omp target data use_device_ptr(B32,QVV)
#endif
#if defined(HIPBLAS) || defined(HIPBLASXT)
#if defined(HIPBLAS)
          hipblas_return =  HIPBLASDGEMM_v2 &
#elif defined(HIPBLASXT)
          hipblas_return =  hipblasXtDgemm  &
#endif
                   (hipblas_handle,HIPBLAS_OP_T,HIPBLAS_OP_N,              &
                     NVIR*iQVV,NVIR,NAUXBASD,                           &
                     1.0D00, B32(1,IACT),NAUXBASD,                                  &
                     B32(1,JACT),NAUXBASD,                                  &
                  0.0D00, QVV,NVIR*iQVV)
        hipblas_return = hipDeviceSynchronize()
